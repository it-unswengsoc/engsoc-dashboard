...
const PORT_LABELS: { value: string; label: string }[] = [
 ...
 */
export async function extractMentionedUserIds(content: string, excludeUserId: number): Promise<number[]> {
  const directory = await listDirectoryUsers();

  const candidates: { phrase: string; userIds: number[] }[] = [];
  ...
  while (true) {
    ...
    for (const candidate of candidates) {
      ...
      if (!restLower.startsWith(phraseLower)) continue;
