...
exports.listRecentFiles = listRecentFiles;
...
async function listPortDirectories() {
    ...
    return Promise.all(allowedDrives.map(async (sharedDrive) => {
...
async function listRecentFiles(limit = 20) {
    ...
    const allowedDriveIds = (drivesRes.data.drives ?? [])
        ...
        .map((d) => d.id);
    const perDriveResults = await Promise.all(allowedDriveIds.map((driveId) => drive.files.list({
