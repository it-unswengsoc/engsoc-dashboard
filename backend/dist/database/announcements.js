 ...
 * none exist).
 */
async function dbGetAllAnnouncements(currentUserId) {
    const result = await pool.query(`${ANNOUNCEMENT_SELECT} ORDER BY a.created_at DESC`, [currentUserId]);
 ...
 * matches.
 */
async function dbGetAnnouncementById(announcementId, currentUserId) {
    const result = await pool.query(`${ANNOUNCEMENT_SELECT} WHERE a.id = $2`, [currentUserId, announcementId]);
 ...
 */
async function dbCreateAnnouncement(input) {
    const insertResult = await pool.query(`INSERT INTO announcements (author_id, content, image_url, created_at, updated_at)
     VALUES ($1, $2, $3, NOW(), NOW())
     RETURNING id`, [input.authorId, input.content, input.imageUrl ?? null]);
 ...
 * existed.
 */
async function dbDeleteAnnouncement(announcementId) {
 ...
 */
async function dbLikeAnnouncement(announcementId, userId) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const insertResult = await client.query(`INSERT INTO announcement_likes (announcement_id, user_id)
       VALUES ($1, $2)
       ON CONFLICT (announcement_id, user_id) DO NOTHING
       RETURNING id`, [announcementId, userId]);
        if ((insertResult.rowCount ?? 0) > 0) {
            await client.query(`UPDATE announcements SET like_count = like_count + 1 WHERE id = $1`, [announcementId]);
        }
 ...
 */
async function dbUnlikeAnnouncement(announcementId, userId) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const deleteResult = await client.query(`DELETE FROM announcement_likes WHERE announcement_id = $1 AND user_id = $2 RETURNING id`, [announcementId, userId]);
        if ((deleteResult.rowCount ?? 0) > 0) {
            await client.query(`UPDATE announcements SET like_count = GREATEST(like_count - 1, 0) WHERE id = $1`, [announcementId]);
        }
        await client.query('COMMIT');
    }
    catch (error) {
